...
OBSBasicStatusBar::OBSBasicStatusBar(QWidget *parent)
	: QStatusBar    (parent),
	  ...
	  kbps          (new QLabel)
{
	sessionTime->setText(PADDING + QString("00:00:00") + PADDING);
	cpuUsage->setText(PADDING + QString("CPU: 0.0%") + PADDING);
...
void OBSBasicStatusBar::UpdateBandwidth()
{
	...
	kbps->setText(PADDING + text + PADDING);
...
void OBSBasicStatusBar::UpdateCPUUsage()
{
	...
	cpuUsage->setText(PADDING + text + PADDING);
...
void OBSBasicStatusBar::UpdateSessionTime()
{
	...
	sessionTime->setText(PADDING + text + PADDING);
...
void OBSBasicStatusBar::UpdateDroppedFrames()
{
	...
	QString text;
	text.sprintf(": %d (%.01f%%)", totalDropped, percent);
	droppedFrames->setText(PADDING + QTStr("DroppedFrames") + text +
			PADDING);
...
void OBSBasicStatusBar::Reconnect()
{
	...
	reconnectMsg.replace("%1", QString::number(retries));
